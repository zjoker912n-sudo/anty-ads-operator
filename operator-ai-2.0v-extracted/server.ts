...
function handleMetaError(res: any, error: any, fallback: () => any) {
  const isAuthError = 
    ...
    error.error_subcode === 460 ||
    (error.message && error.message.toLowerCase().includes('session has been invalidated')) ||
    (error.message && error.message.toLowerCase().includes('validating access token'));
...
async function startServer() {
  ...
  app.post('/api/intelligence/advanced-analysis', async (req, res) => {
    ...
    try {
      ...
      console.error('[Intelligence] AI analysis failed:', error.message);
      res.status(500).json({ error: error.message });
    }
  });
  ...
  app.post('/api/create-checkout-session', async (req, res) => {
    try {
      ...
      if (!stripeSecret) {
        return res.status(400).json({ 
          error: 'Stripe is not configured yet. To receive real payments, please add your STRIPE_SECRET_KEY to the environment variables.' 
        });
      }
      
      const stripe = new Stripe(stripeSecret, { apiVersion: '2023-10-16' as any });
      const { planName, priceAmount, billing } = req.body;
      
      const amountInCents = parseInt(priceAmount.replace(/,/g, '')) * 100;
      const interval = billing === 'per year' ? 'year' : 'month';
        ...
        line_items: [
          {
            price_data: {
              currency: 'usd',
              product_data: {
                name: planName || 'Operator AI Plan',
              },
