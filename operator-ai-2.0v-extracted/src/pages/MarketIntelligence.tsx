...
export function MarketIntelligence() {
  ...
  const analyzeUrl = async (targetUrl: string) => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Failed to parse JSON, returning error", cleanJsonStr);
        resultJson = { error: 'Failed to parse JSON from AI response' };
      }
      
      if (resultJson.error) {
        throw new Error(resultJson.error + (resultJson.result ? ": " + resultJson.result : ""));
      }
  ...
  const tabs = [
    ...
    { id: 'rival-iq', label: 'Rival IQ Benchmarks', icon: BarChart3 },
            ...
            <div className="space-y-12">
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  {activeTab === 'rival-iq' && <RivalIQTab report={activeReport} />}
...
function RivalIQTab({ report }: { report: IntelligenceReport }) {
  return (
    <div className="space-y-12 h-full flex flex-col justify-center">
      <div className="glass-panel p-10 rounded-[3rem] border-indigo-500/20 bg-[#0B0F19] text-center max-w-3xl mx-auto border border-white/5 shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500/5 blur-[80px] -mr-32 -mt-32 pointer-events-none" />
        <div className="w-20 h-20 bg-indigo-500/10 rounded-3xl mx-auto flex items-center justify-center border border-indigo-500/20 mb-8 relative z-10 group hover:rotate-12 transition-transform">
          <BarChart3 className="w-10 h-10 text-indigo-400 group-hover:scale-110 transition-transform" />
        </div>
        <h2 className="text-3xl font-black text-white tracking-tight mb-4 relative z-10">
          Rival IQ Benchmarking Target
        </h2>
        <p className="text-gray-400 leading-relaxed font-medium mb-10 relative z-10">
          Deep social market evaluation requires live connectivity. Initiate a dedicated Rival IQ analytical scan below to benchmark cross-channel engagement for <span className="text-white font-bold">{report.url}</span>.
        </p>

        <div className="relative z-10">
          <button className="px-8 py-5 bg-indigo-600 hover:bg-indigo-500 active:scale-95 text-white font-black rounded-2xl transition-all shadow-[0_0_30px_rgba(79,70,229,0.3)] flex items-center justify-center gap-3 w-full sm:w-auto mx-auto uppercase tracking-wider text-sm">
