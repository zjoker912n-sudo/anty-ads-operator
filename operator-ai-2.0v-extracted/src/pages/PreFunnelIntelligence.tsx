...
type TabType = 'analysis' | 'competitors' | 'trends' | 'funnel' | 'creative' | 'roadmap' | 'rival-iq';
...
export function PreFunnelIntelligence() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      let parsedStrategyRaw: any = {};
      try {
        let cleanStr = data.result || '{}';
        // Strip out markdown formatting if LLM added it
        ...
        parsedStrategyRaw = JSON.parse(cleanStr);
      } catch (e) {
        console.warn("Failed to parse JSON from AI result, using mock template:", data.result);
        parsedStrategyRaw = { error: 'mock' };
      }
      
      // If AI failed or returned error, throw it so the UI shows an error state
      if (parsedStrategyRaw.error) {
        throw new Error(parsedStrategyRaw.error + (parsedStrategyRaw.result ? ": " + parsedStrategyRaw.result : ""));
      }

      // Handle common casing mistakes from LLMs
      const parsedStrategy = {
        businessSummary: parsedStrategyRaw.businessSummary || parsedStrategyRaw.BusinessSummary || {},
        executiveSummary: parsedStrategyRaw.executiveSummary || parsedStrategyRaw.ExecutiveSummary || {},
        detectedProblems: parsedStrategyRaw.detectedProblems || parsedStrategyRaw.DetectedProblems || [],
        competitorInsights: parsedStrategyRaw.competitorInsights || parsedStrategyRaw.CompetitorInsights || { tier1: [], marketBroad: [] },
        marketTrends: parsedStrategyRaw.marketTrends || parsedStrategyRaw.MarketTrends || {},
        funnelStrategy: parsedStrategyRaw.funnelStrategy || parsedStrategyRaw.FunnelStrategy || { platforms: [] },
        creativeAndAdvantage: parsedStrategyRaw.creativeAndAdvantage || parsedStrategyRaw.CreativeAndAdvantage || {}
      };
      ...
      {strategy ? (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-1000">
          ...
          <div className="flex overflow-x-auto border-b border-white/5 no-scrollbar sticky top-0 bg-[#0B0F19]/80 backdrop-blur-md z-30 justify-center">
            ...
            {renderTabButton('rival-iq', 'Rival IQ', <BarChart3 className="w-4 h-4" />)}
          </div>
          ...
          <div className="min-h-[600px]">
            ...
            {activeTab === 'rival-iq' && (
              <div className="space-y-12 animate-in fade-in duration-500">
                <div className="glass-panel p-10 rounded-[3rem] border-indigo-500/20 bg-indigo-500/5">
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                      <h2 className="text-3xl font-black text-white">Rival IQ Benchmark</h2>
                      <p className="text-indigo-200/60 font-medium mt-1">Deep Social & Engagement Analytics for {strategy?.businessSummary?.name || 'Target'}</p>
                    </div>
                  </div>
                  
                  <div className="prose prose-invert prose-indigo max-w-none bg-[#0B0F19] p-8 rounded-3xl border border-white/5 text-right mt-8">
                     <p className="text-gray-400 italic text-center">To initiate a deep competitor social benchmark using Rival IQ, click the button below. This will scan their latest posts, engagement rates, and format effectiveness.</p>
                     
                     <div className="flex justify-center my-8">
                        <button className="px-8 py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-black rounded-xl transition-all shadow-[0_0_30px_rgba(79,70,229,0.3)] flex items-center gap-3 active:scale-95">
                           <Zap className="w-5 h-5 fill-current" />
                           Initiate Rival IQ Scan
                        </button>
                     </div>
                  </div>
                </div>
