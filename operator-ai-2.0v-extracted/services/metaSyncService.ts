...
export class MetaSyncService {
  ...
  static async getWorkspacesToSync() {
    ...
    try {
      ...
      if (err.code === 5 || err.message?.includes('NOT_FOUND') || err.code === 7 || err.message?.includes('PERMISSION_DENIED')) {
        console.warn('[MetaSync] ⚠️ Workspaces collection not found or permission denied. Skipping sync.');
        return [];
      }
